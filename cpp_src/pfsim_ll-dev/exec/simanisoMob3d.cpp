...
int main() {
	...
	const char outdirname[32] = "./../workdir/3danisotestcase/";
	...
	char infilename[32] = "./../data/120erDataSet/ops";
	...
	CHECK_FILE_READ(sim::readcoeffmaps, "../data/coeffmapsAniso/L_0p01_1p3_2p8_31p07.txt", coeffmapsbuffer.data, inccoeff);
	// now repeat for gamma
	CHECK_FILE_READ(sim::readcoeffmaps, "../data/coeffmapsAniso/gamma_0p01_1p3_2p8_31p07.txt", coeffmapsbuffer.data+1, inccoeff);
	...
	for (std::size_t l=1; l<=NUM_ITER; ++l) {
		...
		auto status = sim::thd::stepanisoMob(nopA, valA, idA, 
				nopB, valB, idB,
				deltat,
				misorientation.data,
