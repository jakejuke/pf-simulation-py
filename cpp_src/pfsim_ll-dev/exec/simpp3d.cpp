...
int main() {
	...
	const char outdirname[64] = "./../workdir/iso_0p8_10315p_alt/";
	...
	char infilename[64] = "./../data/120erDataSet/ops";
	...
	CHECK_FILE_READ(sim::readcoeffmaps, "../data/coeffmapsAniso/L_0p8.txt", coeffmapsbuffer.data, inccoeff);
	// now repeat for gamma
	CHECK_FILE_READ(sim::readcoeffmaps, "../data/coeffmapsAniso/gamma_0p8.txt", coeffmapsbuffer.data+1, inccoeff);
	...
	// AUTO NEEDS TO BE DELETED WHEN USING PF OUTPUT
	...
	for (std::size_t l=1; l<=NUM_ITER; ++l) {
		...
		auto status = sim::thd::steppp(nopA, valA, idA, 
				nopB, valB, idB,
				deltat,
				field,
				misorientation.data,
