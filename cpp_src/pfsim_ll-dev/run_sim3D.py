...
# Import my code
import prep_sim3D
PATH_PFSIM = os.path.dirname(os.path.abspath(__file__))
PATH_ANTON = os.path.join(PATH_PFSIM, 'anton')
...
def run_sim3D(h5_fin: str, code_path: str, save_dest: str = 'temp'):
    """Run C++ phase-field code in 3D
    """
    # expand input paths
    h5_fin = os.path.expanduser(h5_fin)
    code_path = os.path.expanduser(code_path)
    save_dest = os.path.expanduser(save_dest)
    
    # make temp dir to store text output files
    if save_dest.casefold() == 'temp'.casefold():
        # create temp directory to save all par files and sim output
        tmp_dir = tempfile.mkdtemp(prefix='pfsim_')
        print('created temporary directory:\n', tmp_dir)
    else:
        # create a folder in user-specified directory
        now = datetime.now()
        output_dir = now.strftime('pfsim_%Y-%m-%d_%Hh%Mm%Ss')
        full_save_dest = os.path.expanduser(save_dest)
        tmp_dir = os.path.join(full_save_dest, output_dir)
        os.makedirs(tmp_dir)
        print('created temporary directory:\n', tmp_dir)
    ...
    [M, N, L] = id_map.shape # dims of id_map (rows = M, columns = N)
    ...
    if np.size(mobility) == 6800:
        mu = mobility
        print('using custom mobilities from h5 input (6800 values)')
    else:
        mu = anton.generate_em('const', mobility)
    anton.moelans(sigma, mu, tmp_dir, PATH_ANTON)

    # prep_sim3D updates C++ files (sim3d.cpp, config3d.hpp, Makefile)
    prep_sim3D.prep_sim(h5_fout)

    ## Run compile C++ code and run simulation
    if DRY_RUN == False:
        print('running phase-field simulation ...')
        # _ _ should use subprocess.call() instead of os.system() _ _
        cwd = os.getcwd()
        os.chdir(os.path.join(code_path, 'exec'))
        # os.system('terminal command')
        os.system('make -B misorientation')
        os.system('make -B sim3di')
        os.system('./sim3di')
        ...
        if save_dest.casefold() == 'temp'.casefold():
            ...
            print('all temp files in:', tmp_dir)

    else:
        print('DRY RUN... skipping simulations')
...
def write_ops(a: np.ndarray, tmp_path: str) -> str:
    ...
    if a.ndim == 2:
        ...
        with open(ff_name, 'w') as f:
            # copied from above to quickly expand to 3D
            for i, j, k in np.ndindex(a.shape):
                x, y, z = (j + 1), (i + 1), (k + 1)
                # write text file with 5 columns: x, y, z, intensity, id
                line = f'{x} {y} {z} 1 {a[i, j, k]}\n'
                f.write(line)
    else:
        raise Exception("Only works for 2D or 3D arrays")
...
def read_ops(f_ops: str, dtype=int, delimiter=None, skiprows=0):
    ...
    num_layers = np.max(a[:,2])
    a_lin = a[:,-1]
    return a_lin.reshape(num_rows, num_columns, num_layers)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Info on how to call this function")
    # parser.add_argument("-f", "--file", default="admin")
    parser.add_argument("-f", "--file", default=None, help="path to input .h5 file")
    parser.add_argument("-c", "--code", default=PATH_PFSIM, help="path to pf_sim code dir")
    parser.add_argument("-d", "--dest", default='~/sim_output', help="path to where temp output is stored")
    ...
    #    # Or for testing uncomment the next line
    #    #run_sim3D('/Users/jules/Code/pfsim_ll/data/ml_leonard/test_file_3d_1.h5')

    print('Now running ...')
    print(f'    run_sim3D({args.file}, {args.code}, {args.dest})')
    run_sim3D(args.file, args.code, args.dest)
    
    # # For testing on my Mac -- also set DRY_RUY to True above
    # run_sim3D('/Users/jules/Code/pfsim_ll/data/test3d_32cube_sIntvls.h5', '/Users/jules/Code/pfsim_ll/', '~/Downloads/foo')
