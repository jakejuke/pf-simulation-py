#ifndef SRC_SIM_STEP2D_HPP
#define SRC_SIM_STEP2D_HPP 1
...
/* list of parameters to specifiy during compile time:
...
int step(std::size_t m, std::size_t n, std::size_t maxnop,
		...
		for (std::size_t j=jzero; j<n+jzero; ++j) {
			...
			*/
			...
			/* propagate each order parameter
				* individually, store result directly
				* into destval;
				*/
...
//wrapper function
template <typename T, typename S, typename R>
int step(mesh::twd::Mesh2d<S> &orignop, 
		...
		return sim::twd::step(orignop.xsize(), orignop.ysize(), origval.opbufsize(),
