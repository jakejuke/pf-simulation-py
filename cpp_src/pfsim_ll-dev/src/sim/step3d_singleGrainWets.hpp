#ifndef SRC_SIM_STEP3DSINGLEGRAINWETS_HPP
#define SRC_SIM_STEP3DSINGLEGRAINWETS_HPP 1

#include <src/mesh/mesh3d.hpp>
#include <src/mesh/compat3d.hpp>
#include <src/aux/ringproject.hpp>
#include <src/sim/packnn3d.hpp>
#include <src/sim/getLcoeff_singleGrainWets.hpp>
#include <src/sim/getGammacoeff_singleGrainWets.hpp>
#include <cstddef>
#include <cstdio>
...
int stepSingleGrain(std::size_t m, std::size_t n, std::size_t k,
			std::size_t maxnop,
		...
		for (std::size_t j=jzero; j<n+jzero; ++j) {
			...
			*/
			...
			for (std::size_t l=lzero; l<k+lzero; ++l) {
				...
				// contribute
				T Lcoeff = sim::get_Lcoeff_singleGrainWets(
									orignop[i*incRowOrignop+j*incColOrignop+l*incLayOrignop],
									misorientation,
									valbuf, 1,
									idbuf, 1,
									coeffdata, incCoeff);
				...
				*/
				...
				for (std::size_t nl=0; nl<orignop[i*incRowOrignop+j*incColOrignop+l*incLayOrignop]; ++nl) {
					...
					T Gammalocal = sim::get_Gammacoeff_singleGrainWets(
						orignop[i*incRowOrignop+j*incColOrignop+l*incLayOrignop],
						nl,
						misorientation,
						valbuf, 1,
...
//wrapper function
template <typename T, typename S, typename R>
int stepSingleGrain(mesh::thd::Mesh3d<S> &orignop,
		...
		return sim::thd::stepSingleGrain(orignop.xsize(), orignop.ysize(), orignop.zsize(),
						origval.opbufsize(),
