#ifndef SRC_SIM_STEPISO2D_HPP
#define SRC_SIM_STEPISO2D_HPP 1
...
/* list of parameters to specify during compile time:
...
int stepiso(std::size_t m, std::size_t n, std::size_t maxnop,
	...
	for (std::size_t i=izero; i<m+izero; ++i) {
		...
		for (std::size_t j=jzero; j<n+jzero; ++j) {
			...
			*/
			...
			// run over all nearest neighbor sites
			// and sum up values with the same id
			// to valbufnn
			...
			// helper variable that might
			// be decremented later
			...
			for (std::size_t l=0; l<orignop[i*incRowOrignop+j*incColOrignop]; ++l) {
				...
				laplacian = (valbufnn[l] - 4 * valbuf[l])
							/ (deltax*deltax);
...
//wrapper function
template <typename T, typename S, typename R>
int stepiso(mesh::twd::Mesh2d<S> &orignop, 
		...
		int c=0;
		if(!mesh::twd::check_ident(orignop, destnop)) c++;
		if(!mesh::twd::check_ident(origval, destval)) c++;
		if(!mesh::twd::check_ident(origval, destval)) c++;
		if(c>0) return(-1);
			
		return sim::twd::stepiso(orignop.xsize(), orignop.ysize(), 
						origval.opbufsize(),
