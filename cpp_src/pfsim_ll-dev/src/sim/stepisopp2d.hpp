#ifndef SRC_SIM_STEPISOPP2D_HPP
#define SRC_SIM_STEPISOPP2D_HPP 1
...
int stepisopp(std::size_t m, std::size_t n, std::size_t maxnop,
		...
		for (std::size_t j=jzero; j<n+jzero; ++j) {
			...
			*/
			...
			for (std::size_t l=0; l<orignop[i*incRowOrignop+j*incColOrignop]; ++l) {
				...
				// sum 
				qsum = fullsum - gammacoeff*valbuf[l]*valbuf[l];
...
//wrapper function
template <typename T, typename S, typename R>
int stepisopp(mesh::twd::Mesh2d<S> &orignop, 
		...
		return sim::twd::stepisopp(orignop.xsize(), orignop.ysize(), 
						origval.opbufsize(),
